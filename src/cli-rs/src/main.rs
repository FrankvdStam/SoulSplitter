...
use soulmemory_rs::{get_process_from_name, get_process_from_names};

use soulmemory_rs::pattern_scanner::{scan, to_pattern};
use soulmemory_rs::pointer::Pointer;
use soulmemory_rs::process::Process;
...
fn main()
{
    ...
    for game in games
    {
        match get_process_from_name(game)
        {
            Ok(mut p) =>
            {
                println!("injecting into {}", p.get_name());
                p.as_mut().inject_dll(&dll_path)
            }
            Err(_) => {}
        }
    }

...
impl Pointer
{
    pub fn new(process: Box<dyn Process>, base_address: usize, offsets: Vec<usize>, is_64_bit: bool) -> Self
    {
        Pointer
        {
            process,
    ...
    fn resolve_offsets(&self, offsets: &Vec<usize>, pointer_path: &mut Option<String>) -> usize
    {
        ...
        for i in 0..offsets.len()
        {
            ...
            if i + 1 < offsets.len()
            {
                ...
                {
                    true => self.process.read_u64(address),
                    false => self.process.read_u32(address) as u64,
                } as usize;
    ...
    pub fn read_i32(&self, offset: Option<usize>) -> i32
    {
    ...
    pub fn read_i64(&self, offset: Option<usize>) -> i64
    {
        let mut offsets_copy = self.offsets.clone();
        if let Some(o) = offset { offsets_copy.push(o); }
        let address = self.resolve_offsets(&offsets_copy, &mut None);
    ...
    pub fn read_u8(&self, offset: Option<usize>) -> u8
    {
        let mut offsets_copy = self.offsets.clone();
        if let Some(o) = offset { offsets_copy.push(o); }
        let address = self.resolve_offsets(&offsets_copy, &mut None);
    ...
    pub fn read_u32(&self, offset: Option<usize>) -> u32
    {
        let mut offsets_copy = self.offsets.clone();
        if let Some(o) = offset { offsets_copy.push(o); }
        let address = self.resolve_offsets(&offsets_copy, &mut None);
    ...
    pub fn read_u64(&self, offset: Option<usize>) -> u64
    {
        let mut offsets_copy = self.offsets.clone();
        if let Some(o) = offset { offsets_copy.push(o); }
        let address = self.resolve_offsets(&offsets_copy, &mut None);
    ...
    pub fn read_f32(&self, offset: Option<usize>) -> f32
    {
        let mut offsets_copy = self.offsets.clone();
        if let Some(o) = offset { offsets_copy.push(o); }
        let address = self.resolve_offsets(&offsets_copy, &mut None);
    ...
    pub fn read_f64(&self, offset: Option<usize>) -> f64
    {
        let mut offsets_copy = self.offsets.clone();
        if let Some(o) = offset { offsets_copy.push(o); }
        let address = self.resolve_offsets(&offsets_copy, &mut None);
    ...
    pub fn read_bool(&self, offset: Option<usize>) -> bool
    {
        let mut offsets_copy = self.offsets.clone();
        if let Some(o) = offset { offsets_copy.push(o); }
        let address = self.resolve_offsets(&offsets_copy, &mut None);

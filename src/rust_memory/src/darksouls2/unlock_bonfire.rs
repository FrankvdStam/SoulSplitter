    ...
    unsafe
        {
            ...
            println!("{} bonfire unlocked {} {}", Local::now().format("%H:%M:%S%.3f"), vec[0], vec[2]);
            //    FLAGS.push(edx);
            //}
        }
...
pub fn init_unlock_bonfire_detour()
{
    unsafe
    {
        ...
        println!("unlock bonfire address: 0x{:x}", address);
        ...
        println!("unlock bonfire enabled: {}", UnlockBonfire.is_enabled());
    }
}

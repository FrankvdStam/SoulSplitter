...
namespace SoulMemory.soulsmods
{
    public static class Soulsmods
    {
        public static void Inject(Process process)
        {
            ...
            var dllPath64 = Path.Combine(basePath, @"x64\soulsmods.dll");
            OverwriteFile("SoulMemory.soulsmods.x64.soulsmods.dll", dllPath64);
        ...
        {
            byte[] buffer;
            using (var stream = typeof(Soulsmods).Assembly.GetManifestResourceStream(manifestResourceName))
            {

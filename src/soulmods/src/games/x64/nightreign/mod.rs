...
#[unsafe(no_mangle)]
#[used]
pub static mut NR_XINPUT_PATCH_ENABLED: bool = false;

#[unsafe(no_mangle)]
#[used]
pub static mut NR_XINPUT_STATE: XINPUT_STATE = XINPUT_STATE {
    dwPacketNumber: 0,
...
pub fn init_nightreign()
{
    unsafe
    {
        ...
        let fn_increment_igt_address = process.scan_abs("increment igt", "f3 0f 59 05 ? ? ? ? 48 8b 0d ? ? ? ? f3 48 0f 2c c0", 8, Vec::new()).unwrap().get_base_address();
